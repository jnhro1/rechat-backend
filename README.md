...
- 인증 — 과제 비목표. userId를 신뢰(CONNECT 헤더)하며 신원 위조(impersonation) 방지는 없음. 단, 읽기 인가(세션 토픽 구독·resume는 활성 참여자만)는 구현됨([§6](#websocket-stomp-endpoint-ws-chat-sockjs))
...
### 5.1 ERD
아래는 관계 개요이며, 컬럼 타입·제약·인덱스를 포함한 전체 DDL은 `src/main/resources/db/migration/V1__init.sql`을 참고하세요.
...
- 읽기 인가: `/topic/sessions/{id}` 구독과 `/app/sessions/{id}/resume`는 해당 세션의 활성 참여자만 허용한다. 비참여자는 구독·resume이 거부되어 대화를 엿볼 수 없다(인터셉터에서 SUBSCRIBE 검사, 컨트롤러에서 resume 검사).
